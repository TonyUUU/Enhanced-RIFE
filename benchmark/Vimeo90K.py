...
try:
    ...
    ENABLE_FLOLPIPS = True
except ImportError:
    print("FloLPIPS module not found. Skip FloLPIPS metric.")
...
for i in f:
    ...
    cv2.imwrite(
        os.path.join(fid_gt_dir, save_name), (gt0 * 255).astype("uint8")
    )
...
outfile = os.path.join(
    "benchmark",
    "result",
    f"vimeo90K_result_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
)
