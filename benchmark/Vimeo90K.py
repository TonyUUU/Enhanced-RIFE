...
for i in f:
    ...
    gt_tensor = torch.tensor(I1.transpose(2, 0, 1)).to(
        device).unsqueeze(0) / 255.
    pred_tensor = torch.round(mid * 255).unsqueeze(0) / 255.
    ssim = ssim_matlab(gt_tensor, pred_tensor).detach().cpu().numpy()

    mid = np.round((mid * 255).detach().cpu().numpy()
                   ).astype('uint8').transpose(1, 2, 0) / 255.
    I1 = I1 / 255.
    psnr = -10 * math.log10(((I1 - mid) * (I1 - mid)).mean())
    psnr_list.append(psnr)
    ssim_list.append(ssim)
    print("Avg PSNR: {} SSIM: {}".format(
